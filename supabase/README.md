...
| 15 | `migration_auth_rls.sql` | Troca RLS `Allow all` por `TO authenticated`; adiciona `movements.created_by`; GRANT dos RPCs a `authenticated` (REVOKE de `anon`) quando as funções existirem. | **PENDENTE — aplicar à mão** depois de criar usuários no painel Auth. Independente de #11–#14 no schema; se #11/#13 ainda não rodaram, os GRANTs de RPC são no-op até existirem as funções. | 2026-08-06 |

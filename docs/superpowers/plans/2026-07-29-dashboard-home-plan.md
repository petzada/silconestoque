...
|---|---|---|
| **Custo-zero nas saídas** (D12) | Sem selo de cobertura | Produto sem `cost_price` congela `unit_value = NULL` e vale R$ 0,00 para sempre (`schema.sql:193-203`). O form de Produto **não tem campo de custo** (`products/page.tsx:855-956`), então só uma Entrada com NF resolve. Catálogo importado grava custo sem NF (`:591`) e portanto sem ponto em `price_history`. **O KPI principal da aba Análise subestima em silêncio.** |
| **Segurança/auth** (D14) | Fora de escopo | Senha em texto plano legível por qualquer um com o anon key (`schema.sql:124`, RLS `Allow all`); cookie de sessão é a constante `silcon_authenticated_2024` (`lib/auth.ts:5`); não existe `middleware.ts` — a guarda é um `useEffect` client-side. `ADR-0004` aceitou o risco condicionado a "URL não divulgada"; a página pública do quiz distribui o anon key e quebrou essa premissa. Mitigação declarada: o quiz será desativado. **Atualizar o ADR-0004 com essa premissa nova.** |
...
Sugestões de Compra com quantidade e valor **negativos** (`purchase-orders/page.tsx:93-95` — a causa some na Fase 0.5) · lixeira do Follow-up é duas ações em uma e torna impossível excluir pedido recebido (`follow-up/page.tsx:562-578`) · `follow_up_solicitations.status` é escrito e nunca lido, com duas implementações divergentes da mesma regra (`:213-221` vs `:81-86`) · Follow-up sem caminho de edição para nada · datas impossíveis aceitas (`:63-69`) · Movimentações não permite editar, e não há trigger de UPDATE — um `PATCH` direto via anon key corrompe o saldo sem autocorreção · exclusão de movimentação é DELETE físico sem trilha · quiz recalcula acerto contra o gabarito atual por posição, então mudar uma questão reescreve notas passadas (`painel/page.tsx:352-354`) · Função (`roles`) não tem tela própria, embora Setor tenha · soft vs hard delete divergente em seis telas · `window.confirm` dentro de um sistema de diálogo desenhado, em 6 arquivos.
